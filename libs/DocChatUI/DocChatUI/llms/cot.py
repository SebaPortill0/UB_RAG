...
from DocChatUI.base import BaseComponent, Document
...
class Thought(BaseComponent):
    ...
    ```python
    >> from DocChatUI.pipelines.cot import Thought
    >> thought = Thought(
...
class ManualSequentialChainOfThought(BaseComponent):
    ...
    `DocChatUI.pipelines.cot.Thought`. Please refer that section for
    ...
    **Create and run a chain of thought without "+" operator:**
    ...
    ```pycon
    >>> from DocChatUI.pipelines.cot import Thought, ManualSequentialChainOfThought
    >>> llm = LCAzureChatOpenAI(...)
    >>> thought1 = Thought(
    >>>    prompt="Word {word} in {language} is ",
    >>>    post_process=lambda string: {"translated": string},
    ...
    **Create and run a chain of thought without "+" operator:** Please refer the
    `DocChatUI.pipelines.cot.Thought` section for examples.
    ...
    llm: LLM = Param(help="The LLM model to use (base of DocChatUI.llms.BaseLLM)")
