...
def export(export_path, output):
    ...
    from DocChatUI.contribs.promptui.config import export_pipeline_to_config
...
def run(run_path, share, username, password, appname, port):
    ...
    from DocChatUI.contribs.promptui.ui import build_from_dict
    ...
    if share:
        ...
        if appname:
            from DocChatUI.contribs.promptui.tunnel import Tunnel
...
def makedoc(module, output, separation_level):
    ...
    Example:
        ...
        \b
        # Make component documentation for DocChatUI library
        $ kh makedoc DocChatUI
    """
    from DocChatUI.contribs.docs import make_doc
...
def start_project(template):
    ...
    which is located at https://github.com/Cinnamon/DocChatUI/tree/main/templates
    ...
    os.system(
        "cookiecutter git@github.com:Cinnamon/DocChatUI.git "
